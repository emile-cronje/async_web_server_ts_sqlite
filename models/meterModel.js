...
FROM (
    ...
    async UpdateMeter(id, meter) {
        await dbConfig_1.default.query("UPDATE meter SET code = ?, description = ?, is_paused = ?, message_id = ? WHERE id = ?", [meter['code'], meter['description'], meter['isPaused'], meter['messageId'], id]);
